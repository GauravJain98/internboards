...
#    permission_classes  = (IsAuthenticated2,)
    queryset = Intern.objects.select_related('user','user__user','user__address').prefetch_related('skills').all()
...
class InternshipReadList(viewsets.mixins.ListModelMixin,viewsmixins.RetrieveModelMixin):
    pagination_class = BasicPagination
    ...
    def get_queryset(self):
        ...
        queryset = Internship.objects.select_related('company','category','company_user').prefetch_related('submission','available','skills').all()
    ...
    def list(self, request, *args, **kwargs):
        ...
        if cache.get(self.__class__.__name__ + str(page) + str(limit)) is None:
            ...
            cache.set(self.__class__.__name__ + str(page) + str(limit),res , 3600*24)
        else:
            res = cache.get(self.__class__.__name__ + str(page) + str(limit))
        return Response(res)
